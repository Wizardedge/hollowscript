...
function c63883999.filter(c,e,tp)
	...
	return lv>0 and c:IsFaceup() and c:IsSetCard(0x45)
		and Duel.IsExistingMatchingCard(c63883999.rfilter,tp,LOCATION_MZONE,0,1,c,tp)
		and Duel.IsExistingMatchingCard(c63883999.spfilter,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_DECK,0,1,nil,lv,e,tp)
end
function c63883999.rfilter(c,tp)
	return c:IsFaceup() and c:IsRace(RACE_FIEND) and c:IsAbleToRemove() and Duel.GetMZoneCount(tp,c)>0
end
...
function c63883999.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c63883999.filter,tp,LOCATION_MZONE,0,1,nil,e,tp) end
...
function c63883999.operation(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		local rg=Duel.SelectMatchingCard(tp,c63883999.rfilter,tp,LOCATION_MZONE,0,1,1,tc,tp)
