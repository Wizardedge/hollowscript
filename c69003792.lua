...
function c69003792.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local b2=Duel.IsExistingMatchingCard(c69003792.RitualUltimateFilter,tp,LOCATION_DECK,0,1,nil,c69003792.rfilter,e,tp,mg,nil,Card.GetLevel,"Equal") and not (ph>=PHASE_DAMAGE and ph<=PHASE_DAMAGE_CAL)
	if chk==0 then return b1 or b2 end
	local s=0
	if b1 and not b2 then
		s=Duel.SelectOption(tp,aux.Stringid(69003792,0))
	end
	if not b1 and b2 then
		s=Duel.SelectOption(tp,aux.Stringid(69003792,1))+1
	end
	if b1 and b2 then
		s=Duel.SelectOption(tp,aux.Stringid(69003792,0),aux.Stringid(69003792,1))
	end
	e:SetLabel(s)
	local g=nil
	if s==0 then
