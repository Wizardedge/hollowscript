...
function c58543073.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,58543073,0x2016,0x1011,0,0,1,RACE_MACHINE,ATTRIBUTE_WIND)
...
function c58543073.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() and not tc:IsImmuneToEffect(e) then
		...
		if c:IsRelateToEffect(e) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			and Duel.IsPlayerCanSpecialSummonMonster(tp,58543073,0x2016,0x1011,0,0,1,RACE_MACHINE,ATTRIBUTE_WIND)~=0 then
